#!/usr/bin/env python
# Created by "Thieu" at 15:25, 07/12/2024 ----------%                                                                               
...
from data_utils import get_bank_marketing
...
def run_trial(opt, seed, data, cf):
    ...
    # Initialize model
    model = NiaRbfClassifier(size_hidden=cf.DATA01['size_hidden'], center_finder=cf.DATA01['center_finder'],
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA01['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_bank_marketing(f"{Config.PATH_READ}/{Config.DATA01['name']}.csv", verbose=False)
    ## Scaling dataset
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA01['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA01['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA01['name']}")
