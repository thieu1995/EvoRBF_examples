...
def run_trial(opt, seed, data, cf):
    ...
    # Initialize model
    model = NiaRbfRegressor(size_hidden=cf.DATA07['size_hidden'], center_finder=cf.DATA07['center_finder'],
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA07['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_superconductivty(f"{Config.PATH_READ}/{Config.DATA07['name']}.csv", verbose=False)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA07['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA07['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA07['name']}")
