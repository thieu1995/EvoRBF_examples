#!/usr/bin/env python
# Created by "Thieu" at 16:25, 07/12/2024 ----------%                                                                               
...
from data_utils import get_rice
...
def run_trial(opt, seed, data, cf):
    ...
    # Initialize model
    model = NiaRbfClassifier(size_hidden=cf.DATA06['size_hidden'], center_finder=cf.DATA06['center_finder'],
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA06['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_rice(f"{Config.PATH_READ}/{Config.DATA06['name']}.csv", verbose=False)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA06['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA06['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA06['name']}")
