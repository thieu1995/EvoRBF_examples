#!/usr/bin/env python
# Created by "Thieu" at 16:08, 07/12/2024 ----------%                                                                               
...
from data_utils import get_car_evaluate
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA03['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_car_evaluate(f"{Config.PATH_READ}/car_evaluate.csv", verbose=False)
    ...
    dt = LabelEncoder()
    y_train_scaled = dt.fit_transform(y_train)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA03['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA03['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA03['name']}")
