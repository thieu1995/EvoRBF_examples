#!/usr/bin/env python
# Created by "Thieu" at 16:56, 07/12/2024 ----------%                                                                               
...
from data_utils import get_ccpp
...
def run_trial(opt, seed, data, cf):
    ...
    # Initialize model
    model = NiaRbfRegressor(size_hidden=cf.DATA08['size_hidden'], center_finder=cf.DATA08['center_finder'],
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA08['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_ccpp(f"{Config.PATH_READ}/{Config.DATA08['name']}.csv", verbose=False)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA08['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA08['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA08['name']}")
