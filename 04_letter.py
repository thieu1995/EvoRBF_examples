...
def run_trial(opt, seed, data, cf):
    ...
    # Initialize model
    model = NiaRbfClassifier(size_hidden=cf.DATA04['size_hidden'], center_finder=cf.DATA04['center_finder'],
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA04['name']}").mkdir(parents=True, exist_ok=True)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA04['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA04['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA04['name']}")
