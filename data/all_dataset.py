#!/usr/bin/env python
# Created by "Thieu" at 22:12, 20/06/2024 ----------%                                                                               
...
def get_dermatology_data():
    ...
    df.to_csv("data/clean/Dermatology.csv", index=False)
    print(X.info())
...
def get_hepatitis_data():
    ...
    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/Hepatitis.csv", index=False)
    return X, y
...
def get_chronic_kidney_disease_data():
    ...
    # Clean y
    y_cleaned = y['class'].str.strip()

    df = pd.concat([X, y_cleaned], axis=1)
    df.to_csv("data/clean/ChronicKidney.csv", index=False)
...
def get_indian_liver_patient_data():
    ...
    X["A/G Ratio"] = imputer.fit_transform(X["A/G Ratio"].to_frame())

    print(X.info())

    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/IndianLiver.csv", index=False)
    return X.values, y.values
...
def get_primary_tumor_data():
    ...
    # df = pd.concat([X, y], axis=1)
    # df.to_csv("data/clean/Tumor.csv", index=False)
...
def get_parkinsons_data():
    ...
    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/Parkinsons.csv", index=False)
...
def get_spect_heart_data():
    ...
    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/Spect.csv", index=False)
...
def get_spectf_heart_data():
    ...
    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/Spectf.csv", index=False)
...
def get_breast_cancer_wisconsin_diagnostic_data():
    ...
    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/BCWD.csv", index=False)
...
def get_breast_cancer_wisconsin_prognostic_data():
    ...
    # Impute missing values
    imputer = SimpleImputer(strategy='mean')
    X["lymph_node_status"] = imputer.fit_transform(X["lymph_node_status"].to_frame())

    print(X.info())

    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/BCWP.csv", index=False)
...
def get_cirrhosis_patient_survival_data():
    ...
    df = pd.concat([X, y], axis=1)
    df.to_csv("data/clean/Cirrhosis.csv", index=False)
...
# get_dermatology_data()
# get_heart_disease()
# get_hepatitis_data()
# get_chronic_kidney_disease_data()
# get_indian_liver_patient_data()
# get_primary_tumor_data()
# get_parkinsons_data()
# get_spect_heart_data()
# get_spectf_heart_data()
# get_vertebral_column_data()
# get_breast_cancer_wisconsin_diagnostic_data()
# get_breast_cancer_wisconsin_prognostic_data()
get_cirrhosis_patient_survival_data()
# get_heart_failure_clinical_data()
# get_national_poll_on_healthy_aging_data()
# get_hcv_data()
# get_aids_clinical_trials_group_study_data()
# get_maternal_health_risk_data()
# get_mammographic_mass_data()
