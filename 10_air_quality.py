#!/usr/bin/env python
# Created by "Thieu" at 17:06, 07/12/2024 ----------%                                                                               
...
from data_utils import get_air_quality
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA10['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_air_quality(f"{Config.PATH_READ}/{Config.DATA10['name']}.csv", verbose=False)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA10['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA10['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA10['name']}")
