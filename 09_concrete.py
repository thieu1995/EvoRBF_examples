#!/usr/bin/env python
# Created by "Thieu" at 17:02, 07/12/2024 ----------%                                                                               
...
from data_utils import get_concrete
...
def run_trial(opt, seed, data, cf):
    ...
    # Initialize model
    model = NiaRbfRegressor(size_hidden=cf.DATA09['size_hidden'], center_finder=cf.DATA09['center_finder'],
    ...
    res = model.evaluate(y_test, y_pred, list_metrics=cf.LIST_METRIC_REG)
...
if __name__ == "__main__":
    ...
    Path(f"{Config.PATH_SAVE}/{Config.DATA09['name']}").mkdir(parents=True, exist_ok=True)

    X_train, X_test, y_train, y_test = get_concrete(f"{Config.PATH_READ}/{Config.DATA09['name']}.csv", verbose=False)
    ...
    df_loss.to_csv(f"{Config.PATH_SAVE}/{Config.DATA09['name']}/{Config.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{Config.PATH_SAVE}/{Config.DATA09['name']}/{Config.FILE_RESULT}", index=False, header=True)

    print(f"Done with data: {Config.DATA09['name']}")
